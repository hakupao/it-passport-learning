...
// S125 ⑤-2 U7 追加: **2022r04-q032** (コンカレントエンジニアリング適用後の業務の流れ 4 図: 直列 / 工程図への戻りループ /
//   全工程の戻りループ / 階段状の重複。dataset のテキスト肢は図に無い注記「（直列）」「（一部並行）」「（全工程並行）」「（縦列）」を加え、
//   イ・ウの戻りループを「並行」、正解肢エの重複を「縦列」と取り違えていた = ②-a 型)。源 page-15 実読で 2×2 配置
//   (ア=左上 / イ=右上 / ウ=左下 / エ=右下) を確認。複合図は上端に題幹 3 行、下端に次問 問33 の 2 行が残るため
//   `topCut: 240` / `bottomCut: 150`。既存 5 題の SPEC・産物は不変。
...
const SPEC = {
  ...
  "2022r04-q032": { topCut: 240, bottomCut: 150 }, // 1432x892、y=90-227 に題幹「問32 コンカレントエンジニアリング…」3 行、図は y=298-678、y=815-892 に次問「問33 …」2 行
};
