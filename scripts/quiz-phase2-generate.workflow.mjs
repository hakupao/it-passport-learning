...
const KEY_GUARD = {
  type: 'object', required: ['figure_derivable', 'derived_answer', 'matches_key', 'note_jp'], additionalProperties: false,
  properties: {
    ...
    note_jp: { type: 'string', description: '不一致/導出不可なら理由 (図の何がそう言うか)。一致なら簡潔に導出根拠' },
  },
}
...
4. key_guard_honest: key_guard が実際に図/stem を見た誠実な評価か (安易な matches_key=true や、明らかな不一致の見落としがない)。
...
const results = await pipeline(
  ...
  async (jp, it) => {
    ...
    for (let round = 1; round <= MAX_ROUNDS; round++) {
      jpReview = await agent(jpReviewerPrompt(inputPath, it.id, it.has_figure, curJp), { label: `rev:${it.id}#${round}`, phase: 'Review', schema: JP_REVIEW_SCHEMA, model: 'opus', agentType: 'oh-my-claudecode:code-reviewer' })
    ...
    if (!curTr) return { id: it.id, key_guard: curJp.key_guard, suspect: !curJp.key_guard.matches_key || !curJp.key_guard.figure_derivable, jp_verdict: jpReview?.verdict ?? 'null', jp_rounds: jpRounds, status: 'translate_failed', has_figure: it.has_figure }
    // TR review + repair
    let trReview = null
    const trRounds = []
    for (let round = 1; round <= MAX_ROUNDS; round++) {
      trReview = await agent(trReviewerPrompt(it.id, curJp, curTr), { label: `trrev:${it.id}#${round}`, phase: 'TR-Review', schema: TR_REVIEW_SCHEMA, model: 'opus', agentType: 'oh-my-claudecode:code-reviewer' })
    ...
    return { id: it.id, key_guard: kg, suspect: !kg.matches_key || !kg.figure_derivable, jp_verdict: jpReview?.verdict ?? 'null', tr_verdict: trReview?.verdict ?? 'null', jp_rounds: jpRounds, tr_rounds: trRounds, has_figure: it.has_figure }
  },
)
