...
const candidates = parsed?.candidates ?? [] // [{id}]
const votes = parsed?.votes ?? 2
if (!inputPath || !Array.isArray(candidates) || !candidates.length) throw new Error('need {input_path, candidates:[{id}], votes?}')
...
const summary = Object.entries(byId).map(([id, rs]) => ({
  id,
  answers: rs.map((r) => r.verified_answer),
  unique: rs.map((r) => r.unique),
  conf: rs.map((r) => r.confidence),
}))
log(`検証完了 ${clean.length}/${tasks.length} (候補 ${candidates.length} ×${votes}票)`)
