    ...
    <div className="flex flex-col h-[calc(100dvh-var(--nav-height,3.5rem))] max-w-3xl mx-auto p-3 sm:p-6 gap-3 sm:gap-5">
      <header className="flex items-start justify-between border-b border-white/[.08] pb-3">
        ...
        <button
          ...
          className="min-h-[36px] px-3 rounded-md bg-white/[.06] border border-white/[.1] text-xs font-medium text-white/50 hover:text-white/90 active:bg-white/[.1] disabled:opacity-40 disabled:cursor-not-allowed transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#e94560]"
        >
          {t("newChat")}
        </button>
      </header>
          ...
          return (
            ...
            >
              <div
                className={
                  isUser
                    ? "bg-[#e94560] text-white rounded-2xl px-3.5 sm:px-4 py-2 max-w-[85%] sm:max-w-[75%] whitespace-pre-wrap text-sm leading-relaxed"
      ...
      <form onSubmit={handleSubmit} className="flex gap-2 safe-bottom">
        <input
          ...
          enterKeyHint="send"
          className="flex-1 h-11 sm:h-10 border border-white/[.12] rounded-xl sm:rounded-lg px-4 sm:px-3 bg-white/[.04] text-white placeholder:text-white/30 text-base sm:text-sm disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#e94560]"
          autoComplete="off"
