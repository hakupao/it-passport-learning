...
| **2** Inline triggers ✅ DONE 2026-05-22 Session 51 | 章末区 (LD-2 chat + quiz buttons scoped to current chapter — reuse existing `<QuizExplain />` modal + dedicated `<ChapterChatModal />` wrapping `useChat` against `/api/chat` with chapter-scope marker) + selection toolbar (new `<SelectionToolbar.tsx>` listening for `selectionchange` → buttons 「译中」「译英」 → opens `<ParagraphTranslate.tsx>` modal calling `/api/chat` with translate prompt) | `apps/web/src/components/{ChapterEndPanel.tsx, ChapterChatModal.tsx, ChapterQuizPicker.tsx, SelectionToolbar.tsx, ParagraphTranslate.tsx}` ✅ + `apps/web/src/lib/book/translatePrompt.ts` ✅ (compose translate prompt + chapter scope marker) + `apps/web/src/lib/book/__tests__/translatePrompt.test.ts` ✅ (14 vitest cases) + `chapterScope.ts` +`buildChapterQuestionSummaries` ✅ (3 new tests) + messages ja/zh/en +17 keys ✅ + ChapterReader.tsx wire-up ✅ | `step_02_triggers/{tree_outline.md, build_log.txt, test_results.txt, design_notes.md}` ✅ | est 2-4h → **actual ~85 min wall (Session 51 single sitting; γ tripwire row #14 = second Phase 3 under-estimate −53% under midpoint; D-094 §2.4 mid-retro pattern continues PLAN.md inline `actual ~85 min` amend NOT full re-estimate)** |
| **3** Progress + Phase 3 close ✅ DONE 2026-05-22 Session 52 | `progressStore.ts` (LD-3 schema: chapters[NN].{scrollY, completedAt} + quiz[qid].{lastAnswered, correct}) ✅ + `<ChapterCompletionGate />` 「我看完了」 button on ChapterReader (LD-3 gate via IntersectionObserver sentinel) ✅ + progress visualization on `BookIndex` (X/16 章完成度 via BookProgressSummary + per-chapter pill via ChapterProgressPill) ✅ + `RETROSPECTIVE_phase3.md` per Rule C ✅ + Phase 3 tag candidate `phase3-α-ship-2026-05-22` (pending user `freeze phase 3 and tag` gate per Session 48 phase-1/2 precedent) + Vercel prod deploy + Playwright e2e smoke (pending user `push it` gate per Sessions 27-51 pattern) | `apps/web/src/lib/book/progressStore.ts` ✅ + `apps/web/src/lib/book/__tests__/progressStore.test.ts` ✅ (28 vitest cases) + `apps/web/src/components/ChapterCompletionGate.tsx` ✅ + `apps/web/src/components/BookProgressSummary.tsx` ✅ (BookProgressSummary + ChapterProgressPill exports) + `ChapterReader.tsx` MOD (mounts gate between ChapterEndPanel and SelectionToolbar) + `BookIndex.tsx` MOD (header BookProgressSummary + per-card ChapterProgressPill) + messages ja/zh/en +13 keys each ✅ | `step_03_progress/{tree_outline.md, build_log.txt, test_results.txt, design_notes.md}` ✅ + `RETROSPECTIVE_phase3.md` ✅ | est 2-3h → **actual ~70 min wall (Session 52 single sitting; γ tripwire row #15 = third Phase 3 under-estimate −53% under midpoint; module Phase-3 N=3 final mean −59%; D-094 §2.4 mid-retro pattern continues PLAN.md inline `actual ~70 min` amend NOT full re-estimate)** |
...
- ✅ **Step 2 Inline triggers DONE** Session 51 close 2026-05-22 — 7 new files (translatePrompt + 4 components + 2 evidence) + 5 modified + 4 evidence files + 17 new vitest cases (cumulative 316 / 316 PASS) + all build/type/lint gates green; in-source LDs LD-Step2-A~H captured in `evidence/phase3/step_02_triggers/design_notes.md §3`; chapter-route bundle +4.4 kB Size / +61 kB First Load (one-time per route entry; useChat + AI SDK shared chunks)
- ✅ **Step 3 Progress + close DONE** Session 52 close 2026-05-22 — 4 new files (progressStore + tests + ChapterCompletionGate + BookProgressSummary) + 5 modified (ChapterReader + BookIndex + 3× messages) + 4 evidence files + RETROSPECTIVE_phase3.md + 28 new vitest cases (cumulative 344 / 344 PASS) + all build/type/lint gates green; in-source LDs LD-Step3-A~H captured in `evidence/phase3/step_03_progress/design_notes.md §3`; bundle Δ: /book +1.13 kB Size / +2 kB First Load (BookProgressSummary + ChapterProgressPill islands), /book/chapter/[nn] +1.00 kB Size / +1 kB First Load (ChapterCompletionGate island), Phase 2 chat surface 169 kB First Load preserved
- **Phase 3 ✅ COMPLETE** — 3 steps DONE; RETROSPECTIVE_phase3.md per Rule C ✅ on disk; Vercel deploy + Playwright + axe-core + Lighthouse pending user `push it`/`freeze phase 3 and tag` gate per Session 48 phase-1/2 freeze precedent
- **γ tripwire** N=3 final: Step 1 −71% / Step 2 −53% / Step 3 −53% (mean −59%); D-094 §2.4 module-level observation = composition-on-frozen-surfaces work plans at midpoint × 0.4 for Phase 4
- **β tripwire** N=14 cumulative unchanged from Phase 2 close; re-opens at user-gated deploy
- Phase 1 + Phase 2 ✅ FROZEN unchanged — tags `phase1-ship-2026-05-19` + `phase2-α-ship-2026-05-21` immutable; D-101 + Steps 1 + 2 + 3 additive composition on top of frozen Phase 2 components
