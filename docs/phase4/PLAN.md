...
| B.4 | Brain | `/api/tutor` endpoint **+ D-105 Phase 2 migrate** | Implement Server Action / Route Handler that takes `TutorContext` + chat history → streams DeepSeek V4 pro response (per D-104 §2.1) via AI SDK v6 `streamText` + `toUIMessageStreamResponse`. Reuse Phase 2 chat handler pattern. D-097 Basic Auth firewall middleware passes through. vitest + handler-level test for cache-block construction. **+ D-105 migrate**: same commit shifts Phase 2 `/api/{chat, quiz/explain, glossary/hover, hello-ai}` from legacy `deepseek-chat`/`-reasoner` → `deepseek-v4-flash` with `providerOptions.deepseek.thinking.type` injection per D-105 §2.1 table. vitest + handler-level tests landed; Playwright + manual smoke deferred to Module D Step D.1 preview deploy per gate G5 batching. Surfaced LD-Module-B-15 (Phase 2 thinking-mode dispatch via call-site `providerOptions` + local `ProviderOptionsShape` type alias + cap pricing tier extension for tutor role) + LD-Module-B-16 (`validateTutorRequestBody` extracted to sibling `tutorRequest.ts` per Next.js route-export restriction; build-time reviewer chain caught the violation). D-095 §2.5(ε) tripwire RESOLVED. D-085 §2.4 frozen surface preserved (middleware 44.2 kB / chat 169 kB / quiz 120 kB / book 121 kB all UNCHANGED at build). | 150-300 min (expanded from 120-240 by D-105 migrate scope) | **~50 min Session 57** | ✅ DONE Session 57 |
| C.1 | Surface | Design lock (D-106 ADR) | D-019 §3a round-1 4Q answered: Q1=standalone `/[locale]/tutor` route, Q2=single global thread `itp:tutor:session:v1`, Q3=snapshot on mount, Q4=auto-detect escalation heuristic. D-106 LOCKED Session 58 Turn 3. | 30-60 min | **~10 min** | ✅ DONE Session 58 |
| C.2 | Surface | Tutor UI component + i18n + persistence + escalation | `<Tutor />` standalone page component + `tutorHistoryStore.ts` (D-106 §2.2 localStorage persistence) + `escalation.ts` `shouldEscalate()` heuristic (D-106 §2.4) + `/[locale]/tutor/page.tsx` route + Tutor.* i18n keys × 3 locales. `useChat` via `DefaultChatTransport({ api: "/api/tutor" })` + per-send body injection `{ tutorContext, escalate }` via `ChatRequestOptions.body`. TutorContext snapshot on mount via `useRef` (D-106 §2.3). | 120-180 min | **~25 min** | ✅ DONE Session 58 |
| C.3 | Surface | Wire to `/api/tutor` + smoke | Wiring landed inside C.2 (Tutor.tsx `useChat` → `/api/tutor` with TutorContext + escalation). Smoke deferred to Module D Step D.1 preview deploy (live streaming verification per PLAN.md §4 G5 batching). | 60-90 min | **~0 min** (merged into C.2) | ✅ DONE Session 58 |
| C.4 | Surface | NavTabs 5th tab | Added `/tutor` to `SECONDARY_TAB_PATHS` + `secondaryLabels` in NavTabs.tsx. Nav.tutor i18n key × 3 locales. Secondary tier per Phase 3 LD-1 visual hierarchy. | 30-60 min | **~5 min** | ✅ DONE Session 58 |
...
| Module B Step B.4 gate (G3) | ✅ FIRED + ✅ CLOSED Session 57 — user `开始 Phase 4 Module B Step B.4` shipped `/api/tutor` route handler + synchronously executed D-105 §2.1 migrate on Phase 2 four routes in atomic commit; **Module B now FULLY DONE** (B.1+B.2+B.3+B.4 all closed); D-095 §2.5(ε) tripwire RESOLVED; D-085 §2.4 bundle invariants preserved at build (middleware 44.2 kB / chat 169 kB / quiz 120 kB / book 121 kB UNCHANGED) |
| Module C gate (G4) | ✅ FIRED + ✅ CLOSED Session 58 — user `开始 C.2 + C.3 + C.4`; D-106 LOCKED (standalone route + global thread + snapshot + auto-detect); C.1+C.2+C.3+C.4 all DONE in one sitting. New surface `/[locale]/tutor` 175 kB First Load + 27 static pages. **Module C FULLY DONE.** |
| Module D gates (G5-G6) | ⏸ downstream — Module D user-pending (`push it` for D.1 preview deploy) |
| Cost tripwire (G7) | ⏸ silent — Module A \$0 + B.1+B.2 \$0 + B.3 ~\$0.35 (across 4 dry-run attempts) + B.4 \$0 (atomic ship + gates only) = cumulative Phase 4 ~\$0.35 vs \$10 tripwire = 28× headroom; vs \$15 cap = 42× headroom |
| RETROSPECTIVE_phase4.md | ⏸ future commitment (Phase 4 close, Module D Step D.3) |
| γ tripwire baseline | midpoint × 1.0 (NOT Phase 3 multiplier) — **revised from Module A + B + C full actuals N=8**: A.1 -72% / A.2 -56% / A.3 -83% / B.1 -67% / B.2 -78% / B.3 **+67% OVER** / B.4 -78% / **C (block) -87%** / mean **-57% under midpoint**. Module C = composition-on-frozen-surfaces profile (Tutor mirrors Chat 1:1 + tutorHistoryStore mirrors historyStore + NavTabs 2-line change) driving extreme under-midpoint like Module A. B.3 remains the only over-midpoint datapoint (new-infra debug). Module D γ datapoints will further calibrate. |
