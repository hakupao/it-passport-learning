...
| 次セッション | **`docs/phase5/PLAN_52_units.md` §0 を読んでから開始。U4 完了 → 次は U5 = 2020r02o 73 問 Sonnet 5 単 pass** (qnums は `full52_population_S118.json` の `exams[].qnums` を**必ず実数で**取る — S124 で PLAN の 59 が誤りだった; `quiz-fidelity-prep-any.mjs 2020r02o u5 <qnums> --precrop` [calib head votes を確認] → workflow `model:sonnet` → `merge-parts --journal` → machdiff → fixer opus [雛形 `quiz-fidfix-S124-u4.mjs`] → reviewer opus 別 type → gate → commit + push)。予算目安 ≈ Sonnet 440 万 + fixer/reviewer opus 各 ≤ 300 万、約 1 h。**起動前に D-146 第 2 条の申告 → ユーザー承認**。**ユーザー判断待ち: D-147 §6 残課題 (2010h22a-q097 / 2018h30a-q081 の字種不統一を LLM 改写で閉じるか受容か)**。⑨ 優先: machdiff 空虚な一致の改修 / 読点脱落 8 field の R8b lane。その後 U6〜U11 → U12。S124 は push 済。 |
