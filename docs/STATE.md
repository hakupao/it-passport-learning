...
| 次セッション | **`docs/phase5/PLAN_52_units.md` (D-146 準拠の unit 計画) を読んでから開始。U0 脚本改修 → U1 Sonnet A/B → U2 波 1〜3 Rule D + push → U3 波 4 resume → U4〜U11 残 8 exam (1 exam/unit) → U12 収尾。** 次 unit の候補順: (0) 冒頭の脚本改修 = `quiz-s7x-fidelity.workflow.mjs` の return を計数 + 要約に限定し audit 全文はファイルへ (主 context 保護) + 題目領域の前裁断 (画像 Read 削減) (1) **Sonnet 5 A/B** = 2015h27a 68 問を Sonnet 単 pass で抄写 → machdiff → Opus 双 pass の 13 欠陥に対する召回 / FP を測る (2) 波 1〜3 の Rule D 審閲 (別 type reviewer、opus) → push (3) 波 4 の resume (2018h30h 14+12 / 2019h31h 14+13 完了済、§42) を A/B 結果のモデルで (4) 波 5〜8。⑨ 残 (S118 §14/§30/§39/§41): N7 `2015h27a-q066` 図裁断破損 / ⑨-d 結合セル markdown 化方針の統一 (波 1 キャプション行 vs 波 2 セル展開) / machdiff 偽陽性 3 型 + prompt 追記 / 判定基準「源に無い記号の挿入も計上」/ N6-b 括弧・引用符・マイナス字形 / `Book` 名前空間 / 共有図組の図側到達率 / `figure_source_corrected` 3 題 / 多図参照 3 題 / q097 en 参照名 / 2 段ヘッダ表の全量走査 / R8 backlog (e) raw 963 field。教訓 (S118 後半): 「全量承認」は「無制限並発の承認」ではない; 主 context は結果本文を受け取らない設計にする; 核験は視覚 OCR 任務で推論負荷が低く、補漏は決定的 diff が担う。 |
