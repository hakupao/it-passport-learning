...
| 次セッション | **S118 完了 (commit 済、push 未)**。次 = **ユーザー判断 1 件**: ⑤-2 全量 (16 exam × 100 問 双 pass ≈ 3200 agent) を行くか、⑤-4 命中 (R8 以外 36 問 + R8b 40 件) の実読 + R8a/R8c 決定的削除で止めるか (S118 log §30 に材料)。判断後は subagent lane + 別 type reviewer + machdiff 常設で。実読 prompt には「検索式 / 正規表現は空白が構文」を明記。⑨ 残 (S118 §14/§30): `Book` 名前空間 / 共有図組の図側到達率不変式 / machdiff 偽陽性 2 型 / N6-a 丸数字区切り 4 問 / N6-b 括弧・読点体裁 / `2014h26h-q054` 腐敗 4 / `figure_source_corrected` 3 題 / 多図参照 3 題 / q028 raw 来歴 / 2015h27a-q089 順序 / Mr. A 性別含意 / 表+源図二重表示 / 選択肢図 a11y。教訓 (S118): 名簿は「evidence が含めないもの」を出所ごとに確認、不変式導入前に現状異常を潰す、源アセット再生成は派生テキスト層の来歴も追う、**「欠落 = 合法既定」の schema は crosscheck では守れず専用 gate が要る**、`source` を書く script は保存式上書き必須、決定的検出器は語義置換に不可視で LLM 核験と補完関係。 |
