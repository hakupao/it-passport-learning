...
| 次セッション | **`docs/phase5/PLAN_52_units.md` §0 を読んでから開始。U1 完了 → 次は U2 = 波 1〜3 の Rule D 審閲 (reviewer 3 体 opus 直列、各波 fixer と別 type) → push**。起動前に D-146 第 2 条の申告 → ユーザー承認。**U2 に先立ち (または U3 の prep として) ⑨ 脚本改修 2 件を推奨**: (a) `quiz-fidelity-merge-parts.mjs --journal <journal.jsonl>` で壊れ part を StructuredOutput から復元 (b) workflow prompt に「`evidence/` と `fidparts/` の他ファイルを読まない」+ 別題 part 参照禁止。U3 以降の抄写は **Sonnet 5 単 pass** (`model:"sonnet"`、70 問 ≈ 420 万 / 87 問 ≈ 520 万 token、既存 Opus 途中分は再利用不可 = 全量新規 run)。以降 U3 波 4 → U4〜U11 → U12。未 push: S119 (3088ec2) + S120。 |
