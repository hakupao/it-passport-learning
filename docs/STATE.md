...
| 次セッション | **① 完了につき、次は残 backlog の優先付け** (ユーザー判断): 候補 = ⑨ の S117 追加分 (特に核験 workflow の機械 diff 化 — 次に核験を回す前に必須)、⑥ 続き (by_year choices 一致チェック、小)、D-143 §6 retrofit、N5 raw stem_jp 一括是正 (6 題、表示無影響)、② 図/シナリオ再抽出 track、⑤ semantic choice-OCR cleanup、⑦ page_image スキーマ、2018h30a page-12 再生成。手順の教訓は S117 log §12 #1 + §14b/§16b/§18b/§20 (note 差し替えはマーカー冪等 + 腐敗を述べた文は全列挙 + NOTE_PRUNE、D-143 §3 boolean 更新を必ず、page 番号は source.page_number、挿入型置換は入力層の既在チェック、translations は phase1 再 merge 禁止)。 |
