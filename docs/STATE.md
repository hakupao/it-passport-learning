...
| 次セッション | **`docs/phase5/PLAN_52_units.md` §0 を読んでから開始。U0 完了 → 次は U1 Sonnet 5 A/B** (2015h27a 68 問、`--precrop` 付き prep → workflow `model:"sonnet"` 単 pass → `merge_cmd` → machdiff → Opus 裁決; 基準 = Opus 双 pass 13 欠陥、判定 = 召回 ≥ 12/13 かつ FP ≤ Opus)。起動前に D-146 第 2 条の申告 (agent 数 / token / 時間 / 停止条件) → ユーザー承認。U1 の最初の数題で part file が絶対パスに書けているか・crop 優先読みが機能しているかを確認してから全量へ。以降 U2 波 1〜3 Rule D + push → U3 波 4 resume → U4〜U11 → U12。run 手順は PLAN §3 (merge_cmd が 1 手増)。⑨ 残は S118 の「次セッション」記載どおり。 |
