...
| 已锁定决定数 | **83** (D-001 ~ D-083；D-083 = Phase 2 high-level direction LOCKED final Session 24 Turn 4 path α user sign-off) |
...
| 下一会话 | **Session 25** — Phase 2 detail-spec session（无固定 #，user 触发）。Entry checklist per D-083 §5.3 + Turn 4 推荐序：(1) §5.5 16 条 mapping → 锁 must-do 子集 / (2) 锁形态主线（intent: **iii+iv** Hybrid+Chat）→ 写 **D-084** / (3) 锁 portability 严格度（intent: **s1** web 起步）→ 写 **D-085** / (4) 锁技术栈 + AI 模型 + 数据源 contract → 写 **D-086** / (5) 锁 LLM 成本预算 + Tier 评估。每个新 D-NNN §1 引用 D-083 §3 intent signal 作为锚点（sub-ADR 路径，D-083 LOCKED final 不变）。建议 entry file `docs/discussion/<date>-session-25.md` + 第一轮 D-019 slow pace 4Q。Session 24 **✅ CLOSED 2026-05-18 Turn 4** (commit 待落)。 |
