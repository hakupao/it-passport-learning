...
| 次セッション | **Phase 2 残 3 回** (`2010h22a` / `2009h21h` / `2009h21a`)。次候補 = 2010h22a / 2009h21h。**常設順序は S114 で 6 点に更新**: ① s7x 保真核験 (双 pass) → 是正 → generate ② trsweep 全 100 問 ③ **caveat スキャンは user-facing + `key_guard.note_jp` の両方** ④ **caveat が指した問は保真核験 workflow に掛け直す** ⑤ 体系 caveat スキャン → explfix → merge → verify-result ⑥ **全是正が終わってから caveat 除去を 1 回で行う** (S114 の取り残しの原因)。注意: 2010h22a は S110/S111 で q091、2009h21a は S111 で answer_keys を是正した回 → prep 後 assert 必須。併せて: D-141 の横断適用 (linkage-gap 8 クラスタ + groups.json のカバレッジ欠落) / explfix の note 更新方針を D で固定 / cross-page 図の prep 改修 (S99 §138) / hosting-housing + 労務派遣 + **中問の zh 訳語揺れ** の横断 D / 既存 18 exam の sidecar 再 merge / 分野見出しの stem 前置 残 20 問。 |
