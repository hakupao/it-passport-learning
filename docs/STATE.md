...
| 次セッション | **横断 backlog 第一步 (④⑥⑧③) は S117 で完了。次に決めるのは ① の pilot 方案** (S117 log §5): (1) C 案 (2 exam で A=s7x 双 pass と B=note 起点掃引を併走し実測で残 14 exam の方式を決める) で行くか、(2) pilot exam = 2019r01a (s7x 25、未核験群最多) + 2024r06 (s7x 8、最少) でよいか、(3) 判定閾値 = B∖A の正解肢上が合計 2 件以上なら B 全展開、でよいか。着手時は Phase 2 常設 8 手順 + **末尾に `node scripts/quiz-keys-crosscheck.mjs` (full) を追加**。D-143 に従い explfix は final 全文書き換え。未 push の S117 commit があるので最初に push gate を確認。 |
