...
| 次セッション | **② = D-144 の 4 段を順に実装** (**段 1 = ✅ S117 完了**: `quizRichText.ts` + `QuizRichText.tsx`、データ不変、全量不変式 + N=21 目視抽検、Rule D FAIL→是正→PASS、log §25/§25a。**段 2 = ✅ S117 完了**: `choice_figures` で ②-a 3 題を選択肢単位の図に、選択肢テキストは三語で中立化 [データ変更あり: 3 問 × 4 肢]、crosscheck A6、Rule D PASS-with-followups 全処置、副次: 2010h22a-q091 の WebP が S111 以来 stale だったのを再生成で是正、log §26)。**段 3(i) = ✅ S117 完了** (決定的スキャナ `quiz-linkage-gap-scan.mjs` で母数を実測 39 問 → 前文複製 4 問 + 共有図挂図 15 問、残 20 = 偽陽性 3 / 再裁断 10 / 源ページ抽出 7、log §27)。**段 3(ii) = ✅ S117 完了**: 前文抽出 15 クラスタ 59 問 (Extract≠Verify 15/15 PASS)、再裁断 12 組 (新規登録 3 組: 2014h26a-mqA/mqB、2014h26h-mqD)、共有図挂図 計 56 問 + 兄弟図 3、分野見出し (中間 26 + 先頭 27 フィールド) 除去、二重段落除去、memo 記号衝突是正、D-143 explfix 1 問。with_fig 464 → 511。**決定的スキャナ v2 の口径では図無し linkage-gap 0** (残 10 は図内ラベルの偽陽性) — ただしマーカー無しの依存は検出できない (Rule D ② で ≥14 問を追加是正)。**次に crosscheck A7「中問の同組全員が前文 probe を含む」を実装**して真の不変式にする。Rule D (段 3(ii)) PASS-with-followups → 全処置 (log §28g)。教訓: 段 3(i) の「不採用 6 組」は連絡表の切れによる誤判定 (Rule D B-1) — 除外判断は原解像度で。log §28、evidence `quiz_chumon_D144s3ii_S117.md`。**段 4 = 次** (3 shell の統一)。**段 4 前の宿題**: vitest に jsdom + `.tsx` include (component 未テスト)、表の a11y (行見出し th scope=row / stem 表の caption)、選択肢図の alt が中立ラベルで読み上げ利用者に無情報 (D-144 の取捨、代替手段を検討)、裁図内字母と字母チップの二重表示。段 2 = `choice_figures` + ②-a 3 題挂図、段 3 = ②-b 軽量組 → 重量組 (exam 単位)、段 4 = 3 shell 統一。進捗は S117 log §25 以降と本欄で追う。残 backlog: ⑤ semantic choice-OCR cleanup、⑦ page_image スキーマ、2018h30a page-12 再生成、D-143 §6 retrofit、⑨ 残り。 |
