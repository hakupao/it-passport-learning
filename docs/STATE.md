...
| 字段 | 值 |
|---|---|
| 最后更新 | 2026-05-12 (**Session 16 OPEN — Step 6.11.D.3 stages 2+3+4 ✅ dispatched & PASS, Gate ② FAIL on D-076 envelope (14 pages)**. User authorized "go D.3" → 跑全 D.3 stack opus 4.7 ($300 soft / $600 hard caps)。Stage 2: 3 attempts (sonnet halt $5 cap → opus halt fail-cap@10 from 5h quota → opus +skip-existing finish 579/579, $112 shadow)。Stage 3: 1 attempt 42 pages opus Vision PASS, $9.77 shadow。Stage 4: 3 attempts (wall-cap 7200 → fail-cap@50 5h quota → finish 554/579, 25 auto-skipped-by-label cover/toc/index/glossary/other), $107.54 shadow, **2226 entities** (chapter/section/term/question/table/figure 6 类齐)。Gate ② first-fire FAIL on checker schema bug (sibling to Session 15 stage1 case — checker 读 `payload.entities[]` 实际是 top-level list)，patch `halt_criteria.py:100-117` list/dict triage + new test。 Gate ② re-fire FAIL on 14 pages with `question.answer_index=-1` (D-076 sentinel) — F-MISTRAL-ANSWER-LINE-LOSS Phase-2 polish 项目复现，Stage 3 heuristic 没抓到 (intersection ∅)，pages 043/064/065/103/134/154/180/181/182/260/300/416/503/525；page_043 是 Session 09b 同一 reproducer。Stage 4 emit -1 是 D-076 设计 (sentinel, 不静默 default)，Gate ② 是兜底哨兵。**Suite 474 → 476** (+2: skip_existing + Gate ② list-schema)。CLI 加 `--skip-existing` (Stage 2) + `--anthropic-soft/hard-usd` (Stage 2) + `--fail-count-soft/hard` (3 stages) + `--wall-time-soft/hard` (3 stages)。 **Cumulative bill: Mistral $0.629 / Anthropic $0 billed / shadow $229.47 / wall 3h16m**。 **No new D**。 2 failure archives written (`failures/stage4_structure/{gate2-checker-schema, d076-envelope-14-pages}.md`)。 **Cannot proceed to D.4 (Stage 4.5 glossary)** — D-073+D-079 Gate ② FAIL = user gate, 4 选项见 session-16 §9 (β.1 force re-OCR 14 / β.2 hand-edit / β.3 加 missing_answer_line detector / β.4 hybrid)。Session 16 待 user 决策；commits 待 user gate (single atomic 推荐)。) |
...
| 下一会话 | **Session 17** — entry = D-076 envelope 14-page remediation（β.1/β.2/β.3/β.4 选一，详见 `failures/stage4_structure/stage4-2026-05-12-d076-envelope-14-pages.md` §"Remediation options"）。Gate ② resolved PASS 后才能进 6.11.D.4 (Stage 4.5 glossary → Gate ③)。Session 16 还需 user gate 关 (commit + remediation choice)。详见 `docs/discussion/2026-05-12-session-16.md` §9。 |
...
| 6.11.D.3 | Stage 2 classify (579 opus) + Stage 3 re-OCR (42 opus Vision) + Stage 4 structure (554 of 579, 25 auto-skipped-by-label, 2226 entities) ✅ all stages PASS clean. Gate ② FAIL on (a) checker schema bug — patched + tested (suite 475→476); (b) 14 pages with `answer_index=-1` D-076 envelope hit, F-MISTRAL-ANSWER-LINE-LOSS reproduce. CLI 加 `--skip-existing/--anthropic-soft-hard/--fail-count-soft-hard/--wall-time-soft-hard` flags. 2 failure archives in `failures/stage4_structure/`. | $0 billed / $228.9 shadow opus | ⚠ Gate ② FAIL — user gate, see Session 16 §9 |
