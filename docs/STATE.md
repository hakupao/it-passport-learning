...
| 已锁定决定数 | **87** (D-001 ~ D-087；D-087 = Phase 2 stack = Next.js 15 + Vercel AI SDK on Vercel (TS strict + SSR) LOCKED final Session 28 Turn 4 path α user sign-off 2026-05-19；sub-ADR of D-086 §2.4 + D-085 §2.5 form-derived) |
...
| 下一会话 | **Session 29 = Phase 2 step 4 cont'd**：锁 **D-088 AI model**（Sonnet 4.6 / Opus 4.7 / Haiku 4.5 + version pin + prompt cache policy detail），基于 D-087 §2.5 Vercel AI SDK + `@ai-sdk/anthropic` 决定。同时考虑顺手锁 **D-089 数据源 contract** + **D-090 LLM cost cap** (复用 D-071 三档 cap，基于 Session 28 β PoC ~$1.22/day cached baseline)。Session 28 ✅ CLOSED 2026-05-19 Turn 4。建议 entry file `docs/discussion/<date>-session-29.md` + 第一轮 D-019 4Q (cadence / 是否打包 D-088+D-089+D-090 / 是否需第二轮 β PoC e.g. Claude Sonnet vs Opus 4.7 quality vs cost spike)。 |
