...
| 已锁定决定数 | **86** (D-001 ~ D-086；D-086 = Phase 2 β-ready portability strictness = s1 strict web stack LOCKED final Session 27 Turn 3 path α user sign-off 2026-05-19；sub-ADR of D-083 §2.3 + §3 intent signal confirmation) |
...
| 下一会话 | **Session 28 = Phase 2 step 4** brainstorm — 锁技术栈 (D-087) + AI 模型 (D-088 候选) + 数据源 contract (D-089 候选) + LLM cost cap for whole-book Chat scope (per D-085 §5.3 risk handoff, 复用 D-071 三档 cap)。Session 27 ✅ CLOSED 2026-05-19 Turn 3。注意 surface 大：framework pick (Next.js / Astro / SvelteKit / 等) + AI provider (Claude / GPT / local) + 数据源 (read-only v1.0.3 zip / static JSON serve / DB ingest) + RAG/chunking strategy + cost cap — 可能要拆 D-087/D-088/D-089 多 ADR；可能要技术 PoC。建议 entry file `docs/discussion/<date>-session-28.md` + 第一轮 D-019 4Q (Round 1 应该考虑 cadence + 是否拆 ADR + 是否要 PoC)。 |
