...
| 已锁定决定数 | **88** (D-001 ~ D-088；D-088 = Phase 2 AI model + version pin + prompt cache policy = Opus 4.7 default + Hybrid pin + system+glossary ephemeral cache + 1-retry no-fallback + δ-all-tripwire LOCKED final Session 29 Turn 6 path α user sign-off 2026-05-19；sub-ADR of D-087 §2.5 + D-085 §2.4 form-derived; informed by γ PoC `evidence/phase2_d088_poc_2026-05-19/`) |
...
| 下一会话 | **Session 30 = Phase 2 step 4 cont'd**：锁 **D-089 数据源 contract**（v1.0.3 corpus 在 Next.js server 端如何加载/缓存 — FS read at server boot / Vercel Blob / DB ingest；manifest 结构；per-scope excerpt 装配；D-088 §2.3 cache 区块边界依此细化）+ **D-090 LLM cost cap**（复用 D-071 三档 cap pattern：候选 soft $3/day → mid $10/day → hard $30/day + per-query hard cap $5，baseline from `evidence/phase2_d088_poc_2026-05-19/cost_table.md` §3 daily mix；**MUST land before OQ-40 β open per D-088 §5.2 High risk**）。Session 29 ✅ CLOSED 2026-05-19 Turn 6。建议 entry file `docs/discussion/<date>-session-30.md` + 第一轮 D-019 4Q (cadence / 是否打包 D-089+D-090 / PoC 需求 — D-089 可能需 corpus-loading PoC 验证 FS vs Blob latency / 颗粒度 g2 vs g3)。step 5 budget + Tier 评估视 Session 30 进度决定包不包进来。 |
