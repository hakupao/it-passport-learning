...
| 6 | Web App 数据統合 | 🔄 **S85: 教科書リーダー v1 完了** + **S86: Quiz Phase 0 完了** + **S87: Quiz Phase 1 (翻訳) pilot** + **S88〜S93: スケール バッチ ×6** (`2026r08`〜`2014h26h` の最新 18 回、D-小6 フルページ併読 + D-小7 repair 語義ガード + D-小8 combiner 組込) (D-136、翻訳済 **19/29 回**・各 100/100 三語・Rule A S88 実効 36/36 [q072 修復]・S89 36/36・S90 38/38・S91 37/37・S92 40/40 [初の medium/high ゼロ]・S93 38/39 [q086 行12 東西転置を footing 分析で捕捉→surgical 是正+独立再検証 PASS、Rule B archive]・Rule D 写審分離・nft IPA0、残 10 回 batch-by-batch スケール。backlog: S93 q093 choices_jp 全面汚染 (→COUNTIF式 再OCR) / q088 choices_jp[ウ] (商品D→C) + S92 q001/q012/q096/q088 再OCR + q025 glossary / S91以前 q069 choices_jp 再OCR + `2019h31h-q061` figure crop ズレ=要ユーザー判断) — 教科書 244 unit 上線。Quiz: D-134(クリーン派生 `data/ip/quiz/` un-gitignore、IPA 条款で公開 OK・出典/改変明記)+D-135(JP-first 2 モード真題面、翻訳/解析は預生成 backfill)。**Phase 0 = 止血+地基 done**: 首页 500 治癒、quiz v1(2900 問/63 主題/29 回/467 figure WebP、出典・reveal)、vitest 446/build/nft IPA=0、Rule D APPROVE。残 Phase 1(翻訳)/2(解析)/3(教科書統合)。glossary/tutor/chat は死語料依存で 500(各自後続サブ段階)。D-133/D-134: 自作 `textbook/`+派生 `quiz/` のみ in-repo |
