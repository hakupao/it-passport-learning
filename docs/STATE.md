...
| 次セッション | **`docs/phase5/PLAN_52_units.md` §0 を読んでから開始。U3b 完了 → 次は U4 = 2019r01a 59 問 Sonnet 5 単 pass** (qnums は `full52_population_S118.json`; `quiz-fidelity-prep-any.mjs 2019r01a u4 <59 qnums> --precrop` [calib ログの head votes を確認、crop 率が低ければ S123 §1 と同型を疑う] → workflow `model:sonnet` → `merge-parts --journal` → machdiff → fixer opus [雛形 `quiz-fidfix-S123-u3b.mjs`] → reviewer opus 別 type → gate → commit + push)。予算目安 ≈ Sonnet 350 万 + fixer/reviewer opus 各 ≤ 300 万、約 1 h。**起動前に D-146 第 2 条の申告 → ユーザー承認**。**U4 前 (または U4 の fixer 内) に処理すること: ⑨ 解説本文の丸数字引用 vs choices 突合 (D-147 で 23 問に拡大、S123 §7)**。その後 U5〜U11 → U12。S123 は push 済。 |
