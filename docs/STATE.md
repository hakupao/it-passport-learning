...
| 次セッション | **`docs/phase5/PLAN_52_units.md` §0 を読んでから開始。U7 完了 → 次は U8 = 2023r05 91 問 Sonnet 5 単 pass** (qnums は `full52_population_S118.json` の `exams[].qnums` を実数で; `quiz-fidelity-prep-any.mjs 2023r05 u8 <qnums> --precrop` [calib head votes と page_mismatch 数を確認 — 頁パリティで多数 skip なら源ページ読みで token 増] → workflow `model:sonnet` → `merge-parts --journal` → machdiff (MISSED は実残差が混じる — U6・U7 で計 6 件) → fixer opus [雛形 `quiz-fidfix-S125-u7.mjs`、既定 dry-run・`--apply`] → reviewer opus 別 type → gate + build → commit + push)。予算 ≈ Sonnet 600 万 + opus 各 ≤ 300 万、≈ 1 h。**起動前に D-146 第 2 条の申告 → ユーザー承認**。ユーザー判断待ち: なし。⑨ 優先: (1) **出荷層の見出し混入 `2019h31h-q001` zh/en + `2015h27a-q089`** (小タスク) (2) precrop 頁パリティ + 題番号照合 (3) 数字列 (斜線ゼロ) の源照合走査 — U5〜U7 で 3 exam 連続 (4) page 1 頁ずれ全 exam 走査 + MOVE_QUESTION 恒久ゲート (5) machdiff 空虚な一致 (6) 読点脱落 8 field R8b lane。その後 U9〜U11 → U12。S125 は push 済。 |
