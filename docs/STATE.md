...
| 次セッション | **`docs/phase5/PLAN_52_units.md` §0 を読んでから開始。U3a 完了 → 次は U3b = 2019h31h 87 問 Sonnet 5 単 pass** (`quiz-fidelity-prep-any.mjs 2019h31h u3b <87 qnums from full52_fidelity_input_2019h31h.json> --precrop` → workflow `model:sonnet` → `merge-parts --journal` → machdiff → fixer opus [雛形 `quiz-fidfix-S122-u3a.mjs`] → reviewer opus 別 type → gate → commit + push)。予算目安 ≈ Sonnet 520 万 + fixer/reviewer opus 各 ≤ 300 万、約 1.5 h。**起動前に D-146 第 2 条の申告 → ユーザー承認**。U3b 前に決めること: 丸数字区切り字種の dataset 全体方針 (`2019h31h-q062` が N6-a 残)。その後 U4〜U11 → U12。S122 は push 済。 |
