...
| 次セッション | **S118 で A7 / ⑨ 遺留 3 件 / page-12 / D-143 §7 完了、commit 済 (push 未)**。次 = **ユーザー判断 2 件** (D-019、S118 log §2/§3 に選択肢): (1) ⑤ の閉じ方 — 推奨 ⑤-3 (層化抽検 3–4 exam × 20 問で 7.5% と正解肢命中の有無を定档) + ⑤-4 (決定的検出器の corpus 全量化) 並行 (2) ⑦ の閉じ方 — 推奨 ⑦-2 (`source.figure_page_image` 追加、既定 = page_image、B6 拡張) を 12 件実読 (⑦-1) と同時に。判断後の実装は subagent lane + 別 type reviewer で。⑨ 残: `Book` 名前空間 / 共有図組の図側到達率不変式 / 2015h27a-q089 設問文順序 / Mr. A 性別含意 / 表+源図二重表示 / 選択肢図 a11y / S117 分の cosmetic 群。手順の教訓 (S118): 名簿を evidence から起こす時は「evidence が何を含めないか」を出所ごとに確認、不変式導入前に現状の異常 (idx≠0) を潰す、源アセット再生成では画像由来テキスト層の来歴 (`stem_resourced_s7x`) も追う。 |
