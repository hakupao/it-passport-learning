...
| 次セッション | **D-144 (②) 全 4 段完了**。次の優先候補 (ユーザー判断): (1) crosscheck **A7**「中問の同組全員が前文 probe を含む」を実装して段 3 の不変式を常設 (2) ⑤ semantic choice-OCR cleanup (16 exam) (3) ⑨: e2e quiz.spec の書き直し (D-085 前提)、`QuizList` i18n 名前空間の整理、2015h27a-q089 の設問文順序、Mr. A 性別含意の訳文方針、表+源図の二重表示、選択肢図の a11y (4) ⑦ page_image スキーマ / 2018h30a page-12 再生成 / D-143 §6 retrofit。手順の教訓は S117 log §25a/§26a/§28e/§28g (除外判断は原解像度で、検出器の口径を明記、前置後の段落重複除去、corpus 差し替え後は dev 再起動)。 |
