...
| 已锁定决定数 | **84** (D-001 ~ D-084；D-084 = v1.0.3 patch release for D11 kana_helper backfill LOCKED final Session 25 Turn 4 path α user sign-off 2026-05-19；sub-ADR of D-083 §2.4) |
...
| 下一会话 | **Session 26 = Phase 2 step 2** brainstorm（v1.0.3 已 shipped 2026-05-19，branch B 关闭）— 锁形态主线（intent: **iii+iv** Hybrid+Chat per D-083 §3）→ 写 **D-085**；建议 entry file `docs/discussion/<date>-session-26.md` + 第一轮 D-019 4Q。后续 step 3 portability → D-086 / step 4 技术栈 + AI 模型 + 数据源 contract → D-087+ / step 5 预算 + Tier。每个新 D-NNN §1 引用 D-083 §3 intent signal (sub-ADR 路径)。Phase 2 形态主线锁完后回头评 §5.5 #4 F-COP21（推 Session 26 mini-decision per Session 25 Turn 3）。 |
