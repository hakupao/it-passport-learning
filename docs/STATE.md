...
| 次セッション | **`docs/phase5/PLAN_52_units.md` §0 を読んでから開始。U5 完了 → 次は U6 = 2021r03 88 問 Sonnet 5 単 pass** (qnums は `full52_population_S118.json` の `exams[].qnums` を必ず実数で取る; `quiz-fidelity-prep-any.mjs 2021r03 u6 <qnums> --precrop` [calib head votes を確認] → workflow `model:sonnet` → `merge-parts --journal` → machdiff → fixer opus [雛形 `quiz-fidfix-S125-u5.mjs`、既定 dry-run・`--apply` で書込] → reviewer opus 別 type → gate → commit + push)。予算目安 ≈ Sonnet 570 万 (6.5 万/問) + fixer/reviewer opus 各 ≤ 300 万、約 1 h。**起動前に D-146 第 2 条の申告 → ユーザー承認**。ユーザー判断待ち: なし。⑨ 優先: machdiff 空虚な一致の改修 / 読点脱落 8 field の R8b lane / 数字列 (斜線ゼロ) の源照合走査。その後 U7〜U11 → U12。S125 は push 済。 |
