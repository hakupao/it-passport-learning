...
### wrong 1 件: 2012h24a-q065 — 3:1 争議として記録（多数維持）
- 審計判定: primary_ok=false。「本題は閾値条件付き価格比較=最適化計算であり、primary=strategy-01-03(会計・財務) より secondary の strategy-01-02(業務分析・データ利活用) が中核に近い疑い」。
- **判定経緯（4 独立判定）**: 双盲 A(general-purpose) + B(explore) + 重判(scientist) の **3 者が strategy-01-03**、審計(code-reviewer) のみ strategy-01-02。
- **処置**: 多数(3:1)を尊重し **primary=strategy-01-03 維持**。審計意見 strategy-01-02 は**重判が既に secondary に保持**済。confidence=**low**（曖昧を明示）。terms は清洗で空（造語除去、後述）。
- 単方で多数判定を覆さない（D-019 慎重原則）。G4 注意清单に含む。将来ユーザー裁定で swap 可。
