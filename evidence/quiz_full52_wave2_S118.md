...
| 2017h29a | 92 | 5 | 6 | 6 | 3 (うち q020 のマイナス項目のみ見送り、q020 自体は「単位：」で採用) | **9** | 9.8 % |
| 合計 | **158** | 9 | 10 | 11 (7.0 %) | 5 | **16** | **10.1 %** |

> S121 Rule D MINOR-1 訂正: 旧記載「2017h29a 8 / 合計 15」は q020 を題単位で減算した誤り。§7 (16 問) / §8 / fixer の FIX キー 16 と整合させた。
...
去重すると **8 題 / 11 field** (うち採用 6 題 / 9 field。S121 Rule D MINOR-2 訂正: 旧記載「6 題 / 12 field」):
...
| q020 | stem | `単位：百万円` | `単位　百万円` (page-10) | — | gp・cr 両 agent が verdict=CLEAN のまま discrepancy に記載 (S118 §44 q079 型の自己矛盾、現行 machdiff は VERDICT_CONFLICT で検出。S121 MINOR-3 訂正: 旧記載「主 context 実読」) | jp 全層 |
...
| 項目 | 理由 |
|---|---|
| `2017h29a-q020` 表の `-20` (U+002D) vs 源のマイナス記号 | 原寸で ASCII ハイフンでないことは明確だが、全角 2 字形 (U+2212 / U+FF0D) のいずれかが**確定できない** (S121 MINOR-4 で理由記述を訂正)。ASCII `-` は語義的に等価で誤読を生まない。字形クラス **N6-b** に登記。「走査で字形が確定できるものだけ是正する」線引きに従う (括弧は確定できたので是正、マイナスは不可) |
...
> **S121 Rule D MINOR-5 (前提の明記)**: `2016h28h-q094` の raw `stem_jp` (3 層) はアンカ自体が源と別の崩れ方 (`『仕入一覧" 表` / `*仕入” 表`) のため sub() が無言 skip、clean 層のみ是正済 (N5 登記)。波 1 `2016h28a-q043` の `.phase1` 未反映と同型。**出荷源は questions.json の clean 層 + translations sidecar であり、raw / `.phase1` からの再 merge は禁止 (S117 §10a 教訓) を前提とする**。再 merge を行う場合は N5 台帳の raw 残穴を先に是正すること。
