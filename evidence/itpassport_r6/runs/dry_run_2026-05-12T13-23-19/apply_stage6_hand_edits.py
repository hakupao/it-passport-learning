#!/usr/bin/env python3
"""Transactional hand-edit apply for Session 20 Q6-A.

Pattern follows Session 19 `apply_hand_edits.py`:
  1. Pre-validate every entry: path resolves, `before` matches current disk
     value byte-for-byte, `after` differs from `before`.
  2. If any entry fails validation, REFUSE to write anything.
  3. Otherwise apply all 5 entries atomically (per-file json read + replace
     + write).
...
SIDECAR = Path(__file__).parent / "step_06_11_D_6_hand_edits.json"
TRANSLATED_DIR = RUN_DIR / "translated"


def walk(d, path):
    """Walk a nested dict/list following the path; return (parent, last_key)."""
    for key in path[:-1]:
        d = d[key]
    return d, path[-1]
...
def validate(entries):
    """Per-entry validation; collect all errors and return (ok, errors)."""
    errors: list[str] = []
    for e in entries:
        eid = e["id"]
...
def apply(entries):
    """Apply each entry; assumes validate() has already passed."""
    applied: list[str] = []
    for e in entries:
        page_file = TRANSLATED_DIR / e["page"]
        data = json.loads(page_file.read_text(encoding="utf-8"))
...
def post_verify(entries):
    """Re-read disk and confirm every after-value is in place."""
    mismatches: list[str] = []
    for e in entries:
        page_file = TRANSLATED_DIR / e["page"]
        data = json.loads(page_file.read_text(encoding="utf-8"))
...
def main() -> int:
    ...
    print(f"[apply_stage6_hand_edits] {len(entries)} entries loaded from "
          f"{SIDECAR}")

    ok, errors = validate(entries)
    if not ok:
        print(f"[apply_stage6_hand_edits] VALIDATION FAILED "
              f"({len(errors)} error(s)):")
        for err in errors:
            print(f"  - {err}")
        return 1
    print(f"[apply_stage6_hand_edits] validation passed for all "
          f"{len(entries)} entries")

    if args.dry_run:
        print("[apply_stage6_hand_edits] --dry-run: no writes performed")
        return 0

    applied = apply(entries)
    print(f"[apply_stage6_hand_edits] applied {len(applied)}/{len(entries)}: "
          f"{applied}")

    mismatches = post_verify(entries)
    if mismatches:
        print(f"[apply_stage6_hand_edits] POST-VERIFY FAILED:")
        for m in mismatches:
            print(f"  - {m}")
        return 2
    print(f"[apply_stage6_hand_edits] post-verify clean: "
