#!/usr/bin/env python3
"""Step 6.11.E.2 release publish invocation (Session 22 / same Session 21 continuation).

Calls `cert_extractor.release.publish()` with PublishInputs assembled from:
  - data/.../output/ (Stage 7 emitted artifacts — 4 top-level + pages/)
  - data/.../cost.json (run cost ledger)
  - data/.../output/glossary.json (term count)
  - data/.../output/index.json (page/entity totals + Stage 6 summary)
  - data/.../output/polish_items.json (flattened by_page + run_level)
  - docs/release-notes/itpassport-r6-v1.0.0-intro.md (Session 14 C.4)
...
Two modes:
  ...
  --confirm  → stages assets THEN runs `gh release create` + `gh release view`,
              producing the public GitHub Release `itpassport-r6-v1.0.0`.
...
INTRO_MD = REPO_ROOT / "docs/release-notes/itpassport-r6-v1.0.0-intro.md"
EXTRACTOR_SRC = REPO_ROOT / "packages/extractor/src"

# Make sure the extractor package is importable when running this script
# directly (uv run sets this up automatically; bare python needs the hint).
...
def _build_inputs() -> PublishInputs:
    ...
    # notes.py expects flat keys; index.json from Stage 7 nests them under
    # `totals`. Map for compose_notes consumption.
    index_for_notes = {
        **index_raw,
    ...
    cost_for_notes = {
        ...
        "anthropic_usd_billed": 0.0,  # max-plan OAuth per D-069 = $0 billed
