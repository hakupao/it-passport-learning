...
- **Web app**: Next.js 15 at `apps/web/` — quiz, glossary, chat, AI tutor (4 features × 3 themes × 3 locales)
- **Phase 5**: IPA 官方源 (シラバス + 過去問) → Claude vision 提取 → AI 生成三语教科書
...
```
/
├── apps/web/              # Next.js 15 app (pnpm workspace)
│   ├── src/app/           # App Router routes (chat/quiz/glossary/tutor)
│   ├── src/components/    # UI components (3 theme shells)
│   ├── src/lib/           # Data layer, AI, tutor, quiz, glossary, chat
│   ├── e2e/               # Playwright e2e tests
│   └── messages/          # i18n JSON (ja/zh/en)
├── scripts/               # Phase 5 extraction scripts (TS)
├── docs/
│   ├── STATE.md           # Live state (read first!)
│   ├── decisions/         # Active ADRs (D-082+)
│   ├── discussion/        # Active session logs (53+)
│   ├── phase5/PLAN.md     # Current phase plan
│   ├── archive/           # Historical docs (sessions 1-52, Phase 1 ADRs, etc.)
│   └── templates/         # Evidence/failure templates
├── evidence/              # Rule A audit evidence (committed)
├── failures/              # Rule B failure archive (committed, never deleted)
├── data/                  # Pipeline data (gitignored)
├── .source/               # Copyrighted source materials (gitignored)
├── CLAUDE.md / AGENTS.md
└── RETROSPECTIVE*.md      # Phase retros (Rule C)
```

- **TypeScript only** (D-110). No Python toolchain.
- `apps/web/` is the sole pnpm workspace member.
- All pipeline data under `/data/` (gitignored). Final output via GitHub Release + git tag.
